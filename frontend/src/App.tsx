...
function App() {
  ...
  const stats = useMemo(
    () => ({
      ...
      claimed: safeAchievements.filter((achievement) => achievement.claimed).length,
      pending: safeAchievements.filter((achievement) => !achievement.claimed).length,
    }),
    [achievements]
  );
...
const goToApp = () => navigatePage("dashboard");
...
const connectWallet = async() =>{
  try{
    ...
    await fetch("http://127.0.0.1:8000/api/connect-wallet/", {
      method: "POST",
      headers: {
...
function LoginPage({ onLogin, onSignupClick,}: { onLogin: (userData: any) => void; onSignupClick: () => void; }) {
